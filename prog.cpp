			...
			if (n > 2)
			{
				...
				for (int l = 0; l < n; l++) //строки
				{
					for (int k = 0; k < n; k++) //столбцы
					{
						...
						for (int j = 0; j < n - 1; j++) //идёт по строкам
						{
							for (int i = 0; i < n - 1; i++) //идёт по столбцам
							{
								if (j < l && i < k)
								{
						...
						tmp1.set(k, l, tmp2.determinant() * powf(-1, l + k + 1) / fabs(det1));//элементы матрицы алгебраических дополнений
					}
				}
